...
2026-08-30 — tanda de 7 ideas del backlog de Mike (rama `objetosMike`): gate de audio, modo difícil, teclado, tarjeta compartible, desafío diario, pantalla de nombre y borrado manual:

Se implementaron 7 de las 15 ideas de `docs/lista_ideas.md` (las 1, 3, 5, 10, 11, 12 y 13), las que no dependen de arte nuevo. El trabajo arrancó en una rama de pruebas (`pruebasMike`, sacada de `develop`) y al terminar se llevó todo a `objetosMike` (que apuntaba al mismo commit, así que fue un `checkout` limpio arrastrando los cambios sin commitear) y se borró `pruebasMike`. Dos archivos nuevos en `src/ts/`: `random.ts` y `keyboardNav.ts`.

**Idea 1 — gate de audio.** Pantalla negra con un único botón `Start` (`#start-gate-screen`, primer hijo del `<body>`) antes del menú. `#menu-screen` pasó a arrancar con `class="hidden"` y `currentState` inicial es `"start-gate"` (nunca es destino de `changeState()`, solo el estado del HTML). Se borró de `main.ts` el `musicManager.playMenu()` suelto del arranque y el listener de `pointerdown` de respaldo: el click en un botón real ya cuenta como gesto de usuario válido para el navegador, así que la música del menú arranca sin reintentos. Resultado neto: menos código. Mismo criterio de siempre en el CSS — el centrado del botón lo resuelve `#start-gate-inner` (position + flex), nunca un `display` directo por ID en la sección.

**Idea 3 — modo difícil.** `Game` ganó un 3er parámetro opcional `hardMode = false`: `#maxErrors` pasa de 4 a 3, y `VisitorGenerator.generate()` recibe el flag y le suma `+0.15` a `problematicRatio` (con tope 0.95). Al ser opcional con default sin efecto, todo el código que llamaba `new Game(nombre)` / `new Game(nombre, dias)` — los 28 tests incluidos — sigue idéntico. Getters nuevos `maxErrors` / `hardMode`; el HUD y el resumen entre días ahora muestran `X / maxErrors` (no `/ 4` fijo) y el umbral de alerta rosa es `maxErrors - 1`. Persistencia con el mismo patrón `?? false` que ya usaba `totalDays`: se guarda en la partida en curso y `loadProgress()` lo restaura (recalculando `#maxErrors`), así retomar una partida difícil no la convierte en normal. El modo se elige con un **toggle en la barra lateral del menú** (`#hard-mode-toggle-btn`, "Modo difícil: OFF/ON") — solo desde ahí, nunca a mitad de partida; mientras está activo se deshabilita el slider de duración del día y se fuerzan 30s. En el historial se agregó una **columna "Modo"** (`Difícil` / `Normal`), marcada en `saveToHistory()` tanto en la rama de victoria (`endDay()`) como en la de derrota (`decide()`); las partidas guardadas de antes de este cambio se leen como "Normal". `#paused-status` también muestra `· Difícil`.

**Idea 5 — teclado para los botones (no en la pantalla de juego).** Módulo nuevo `keyboardNav.ts`: las flechas (↑↓←→) mueven el foco entre los botones visibles de la pantalla activa, y Enter/Espacio los activan (comportamiento nativo de `<button>`, no hace falta handler propio). Se saltea `#game-screen` a propósito (ahí los sellos son drag and drop con el mouse, ver `stampDrag.ts`) y no interfiere si el foco está en un input de texto o un slider (ahí las flechas son para el cursor / el valor). `changeState()` deja el foco en el primer botón de la pantalla nueva — o en el input de texto si lo hay — así "Continuar", "Empezar día", "Iniciar partida" y demás se usan sin tocar el mouse. Los elementos ocultos se descartan con `getClientRects().length`.

**Idea 10 — tarjeta de resultado compartible.** `buildResultCard(game)` en `main.ts` arma un texto corto tipo Wordle (resultado, día alcanzado, errores, dinero, `🔥 Difícil` si corresponde, y un emoji por cada tipo de yokai que se coló) leyendo solo getters que `Game` ya expone — cero campos nuevos en `Game`. Botón `#share-result-btn` en `#final-screen`, visible únicamente en el último cuadro del final (junto a "Volver al menú"). Copia con `navigator.clipboard.writeText(...)` resuelto con `.then()/.catch()` (nada de `async`/`await`, ver `convenciones.md`), con guarda por si `navigator.clipboard` no existe (contexto no seguro); feedback cambiando el texto del botón a "¡Copiado!" un instante, sin agregar nada al DOM.

**Idea 11 — desafío diario con semilla fija.** Archivo nuevo `random.ts`: `mulberry32(seed)` (PRNG determinista, ~10 líneas, devuelve una función con la forma de `Math.random`) y `todayChallengeSeed()` (la fecha local como número `YYYYMMDD`). `Game` y `VisitorGenerator` ganaron un parámetro `randomFn = Math.random`: **todos** los `Math.random()` de `VisitorGenerator` (rostro, ojos, región, sello, si es problemático, si es alien, frases, cara de alien — ~21 usos) pasaron a `this.#random()`. Cambio mecánico, no de lógica: sin `randomFn` el comportamiento es byte-idéntico al de siempre, por eso ningún test cambia. La semilla se deriva de la fecha **en `main.ts`** — `Game` sigue sin saber nada de fechas ni de "desafío". Botón **"🗓️ Desafío diario"** en la barra lateral: arranca una partida de configuración fija (7 días, modo normal, 90s) con `mulberry32(todayChallengeSeed())`, así todos los que jueguen el mismo día calendario enfrentan exactamente los mismos visitantes en el mismo orden. El submit del nombre y este botón ahora comparten un helper `beginGame(name, totalDays, hardMode, randomFn, dayDurationMs)`.

**Idea 12 — arreglar la pantalla de "Introduce tu nombre".** `#name-entry-screen` pasó a usar `.screen-panel` como el resto de las pantallas chicas (antes era un `<h2>` y un `<form>` sueltos sobre el fondo). Se agregó el `<label for="player-name-input">` real, así la regla CSS `#player-name-form label` que estaba huérfana empieza a aplicarse (además de ser lo correcto por accesibilidad). El handler de `#new-game-btn` precarga el input con `loadPlayerName()` — quien ya jugó no re-tipea el nombre. Y si el input queda vacío, la partida usa **`Un1c0rN10`** por defecto (también es el placeholder ahora).

**Idea 13 — borrar partidas guardadas y créditos.** `Storage.ts` ganó `clearCredits()` (`localStorage.removeItem(CREDITS_KEY)`, sin tocar nada más). En `#credits-screen`, dos botones nuevos: "Borrar partidas guardadas" (llama a la `clearSavedGames()` que ya existía — partida en curso + historial + rachas — y refresca la tabla y el botón Continuar) y "Borrar créditos" (`clearCredits()` + re-render de la lista). Cada uno con un `window.confirm()` nativo antes de actuar: son las dos primeras acciones destructivas que el jugador dispara él mismo desde un botón, y no hay sistema de modal propio en el proyecto. Cero cambios en `Game.ts`.

**Verificación**: `tsc` compila limpio y los 28 tests de `docs/test-temporal.mjs` siguen pasando sin cambios (52 marcadores `[OK]`, exit 0). Además se corrió un smoke test aparte (fuera del repo) que confirma: `mulberry32` determinista y en rango, misma semilla → misma secuencia de visitantes, `hardMode` con `maxErrors` 3 y derrota al 3er error, roundtrip de persistencia de `hardMode`, `new Game()` sin cambios (maxErrors 4 / hardMode false), `clearCredits()`, y el historial marcando `hardMode` en la victoria. **No se probó en el navegador** (no había extensión de Chrome disponible en la sesión; Mike lo prueba él).

**Pendientes / notas**:
- Los botones "Desafío diario" y "Modo difícil" viven en `#menu-sidebar`, que está oculta en `<900px` (igual que la tabla de partidas) — quedan solo en desktop.
- El desafío diario **no tiene ranking ni marca propia** todavía: la partida se guarda en el historial como cualquier otra, sin distinguirse como "desafío del 30/08". Es la parte "nice to have" que la idea 11 dejaba para después (campo `challengeDate` + pantalla de comparación).
- Si se pausa y se retoma un desafío diario con "Continuar", la semilla no se persiste, así que a partir de ahí la partida vuelve a usar `Math.random()` y deja de ser la secuencia compartida.
- El toggle de modo difícil no tiene desbloqueo ("solo elegible después de ganar una vez", como sugería la idea): quedó como toggle libre.

---

2026-08-30 (parte 2) — ajustes pedidos por Mike sobre la tanda anterior: se saca la pista de la tienda, se saca la penalización por apurarse, y el botón del último día:

**Se elimina "Revisar pasaporte" de la tienda.** Era la compra más barata (la pista, `buyHint()`): revelaba con un resaltado qué campo del pasaporte o rasgo del personaje violaba una regla. Se borró entero: `Game.buyHint()` y `Game.hintCost`, `Economy.hintCost` y `HINT_COST`, el botón `#shop-hint-btn` y su listener + `hintTargetSelector()` + la constante `HINT_HIGHLIGHT_MS` en `shop.ts`, la limpieza de `.hint-highlight` en `renderVisitor()`, y la regla `.hint-highlight` + `@keyframes hint-glow` en `style.css`. La tienda queda con 2 ítems: **+15 segundos** e **Indulto**.

**Se elimina el cobro de "descuido" por decidir rápido.** Si alguien resuelve al toque de abrir el pasaporte no hay por qué descontarle: decidir rápido no es un error. Se sacó todo el mecanismo `wasRushed`: el 3er parámetro de `Game.decide()` (vuelve a ser `decide(accept, usedAlienStamp = false)`), `Economy.recordRushPenalty()` / `RUSH_PENALTY` / `#dayRushPenalty` / `#lastDayRushPenalty` / `lastDayRushPenalty`, la línea "Descuido (decidiste sin revisar)" del resumen de fin de día, y en `main.ts` el `passportOpenedAt` / `RUSH_THRESHOLD_MS` / el cálculo de `wasRushed` en `resolveDecision()`. Como `passportOpenedAt` solo servía para esto, `initStampDrag()` perdió su 3er parámetro `onPassportOpened` (el pasaporte se sigue abriendo con click igual, solo que ya no avisa un timestamp).
