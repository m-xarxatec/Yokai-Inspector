...
function updateHardModeButton() {
    ...
    const durationSlider = document.querySelector("#day-duration-slider");
...
function renderEndingBeat() {
    ...
    if (shareBtn !== null)
        shareBtn.classList.toggle("hidden", hasMoreBeats);
}
...
function buildResultCard(finished) {
    ...
    const leaked = [];
    if (finished.letThroughOni)
        leaked.push("👹");
    if (finished.letThroughKitsune)
        leaked.push("🦊");
    if (finished.letThroughKappa)
        leaked.push("🐸");
    if (leaked.length > 0) {
