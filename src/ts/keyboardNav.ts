...
function visibleScreen(): HTMLElement | null {
  const screens = Array.from(document.querySelectorAll<HTMLElement>("section"));
...
export function focusFirstControl(screenId: string): void {
  ...
  const screen = document.querySelector<HTMLElement>(`#${screenId}-screen`);
  if (screen === null) {
    return;
  }
...
export function initKeyboardNav(getState: () => string): void {
